...
if ($_SERVER['REQUEST_METHOD'] === 'PUT') {
    ...
    $id = (int)$input['id'];
    $product = $db->quote($input['product']);
    $quantity = (int)$input['quantity'];
    $price = (float)$input['price'];
    $orderDate = $db->quote($input['orderDate']);

    $query = "UPDATE orders SET product = $product, quantity = $quantity, price = $price, orderDate = $orderDate WHERE id = $id";
    
    $result = $db->query($query);

    if (PEAR::isError($result)) {
        die(json_encode(["error" => "Hiba frissítés közben: " . $result->getMessage()]));
    } else {
