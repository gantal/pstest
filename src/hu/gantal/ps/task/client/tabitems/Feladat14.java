...
public class Feladat14 extends TabItem {
	...
	private static final String DELETE_URL = GWT.getHostPageBaseURL() + "feladat14_delete.php";
	...
	private PagingLoader<PagingLoadResult<BaseModelData>> setupLoader() {
		...
		loader.addListener(Loader.Load, new Listener<LoadEvent>() {
		    ...
		    public void handleEvent(LoadEvent event) {
		    	BasePagingLoadResult<BaseModelData> result = (BasePagingLoadResult<BaseModelData>) event.getData();
		    	List<BaseModelData> items = result.getData();
		        for (BaseModelData item : items) {
		            if (item.get("id") instanceof Double) {
		                item.set("id", ((Double) item.get("id")).intValue());
		            }
		        }
		    }
		});
	    ...
	    Button buttonDelete = new Button("Törlés", new SelectionListener<ButtonEvent>() {
	        ...
	        public void componentSelected(ButtonEvent ce) {
	            ...
	            for (ModelData item : selectedItems) {
	                idsToDelete.add((Integer) item.get("id"));
	            }
	...
	private void sendDeleteRequest(List<Integer> idsToDelete) {
	    RequestBuilder builder = new RequestBuilder(RequestBuilder.DELETE, DELETE_URL);
	    builder.setHeader("Content-Type", "application/json");

	    try {
	        StringBuilder jsonArray = new StringBuilder("[");
	        for (int i = 0; i < idsToDelete.size(); i++) {
	            jsonArray.append(idsToDelete.get(i));
	        ...
	        builder.sendRequest(jsonArray.toString(), new RequestCallback() {
	            @Override
	            public void onResponseReceived(Request request, Response response) {
	            	   if (response.getStatusCode() == 200) {
	                       loader.load();
	                       MessageBox.info("Siker", "Sikeres törlés!", null);
	                   } else {
	                       MessageBox.alert("Hiba", "Hiba lépett fel a törlés során! Kód: " + response.getStatusCode(), null);
	                   }
	            }

	            @Override
	            public void onError(Request request, Throwable exception) {
	               MessageBox.alert("Hiba", "Hálózati hiba történt: " + exception.getMessage(), null);
	            }
	        });

	    } catch (RequestException e) {
	    	MessageBox.alert("Hiba", "Hálózati hiba történt: " + e.getMessage(), null);
	    }
	}
